 ...
 * 
 * This module solves the Reindeer Maze challenge. The challenge involves finding
 * the minimum-cost path through a maze where:
 * - Moving forward costs 1 point
 * - Rotating 90 degrees costs 1000 points
 * 
 * Part 1: Find the minimum possible score to reach the end
...
(** Move forward one step in the current facing direction if possible
    ...
    @return Some new_position if the move is valid, None if blocked by a wall or boundary
*)
let move_forward pos maze =
  ...
  then
    Some { pos with row = next_row; column = next_col }
  else
    None
...
module PositionMap = Map.Make(struct
  ...
  let compare p1 p2 =
    ...
    if row_comparison <> 0 then row_comparison else
    let col_comparison = compare p1.column p2.column in
    if col_comparison <> 0 then col_comparison else
...
let find_shortest_paths start edges =
  ...
  let rec process_queue () =
    ...
    | Some (dist, pos) ->
        ...
        let current_dist = try PositionMap.find pos !distances with Not_found -> max_int in
