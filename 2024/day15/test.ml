...
let move_right_print map =
  ...
  ( ( ( map |> fun m ->
        print_grid "Before reverse" m;
        reverse m )
    |> fun m ->
      print_grid "After first reverse" m;
      move_left m )
  |> fun m ->
    print_grid "After move_left" m;
    reverse m )
  |> fun m ->
  print_grid "Final" m;
  m
...
let move_down_print map =
  ( ( ( map |> fun m ->
        print_grid "Initial" m;
        transpose m )
    |> fun m ->
      print_grid "After transpose" m;
      move_right m )
  |> fun m ->
    print_grid "After move_left" m;
    transpose m )
...
let make_move_test name input movement expected =
  ...
  let expected_map = parse_map expected in
  (* Use existing parse_map *)
...
let movement_tests =
  [
    make_move_test "move_left test" "#..OO@." move_left "#.OO@..";
    make_move_test "move_right test" "#.@OO.." move_right_print "#..@OO.";
  ...
  try
    ...
    if has_input then
      ...
      if String.length input > 0 then (
        ...
        parsed_input |> part1 |> Printf.printf "\nPart 1: %d\n";
        parsed_input |> part2 |> Printf.printf "Part 2: %d\n";
