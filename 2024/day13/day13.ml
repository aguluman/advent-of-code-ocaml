(** Day 13: Claw Contraption Solving a puzzle about claw machines where:
    - Each machine has two buttons (A and B) that move the claw in X,Y
      directions
...
(** Find [minimum value] in a list safely.
    ...
    @return
      Option containing minimum cost [Some] or [None] if no solution exists. *)
let find_minimum_cost values =
  match values with
  | [] -> None
  | costs -> Some (List.fold_left min (List.hd costs) (List.tl costs))

(** Calculate minimum tokens needed for Part 1. Tries combinations of button
    presses (max 100 each).
...
let calculate_minimum_tokens machines =
  machines
  |> List.fold_left
       (fun total_tokens { button_a; button_b; prize } ->
         let possible_costs =
           List.init 101 (fun press_a ->
               List.init 101 (fun press_b ->
                   let final_x =
                     (button_a.move_x * press_a) + (button_b.move_x * press_b)
                   in
                   let final_y =
                     (button_a.move_y * press_a) + (button_b.move_y * press_b)
                   in
                   if final_x = prize.target_x && final_y = prize.target_y then
                     Some ((press_a * 3) + press_b)
                     (* Cost calculation: A=3 tokens, B=1 token *)
                   else None))
           |> List.concat
...
(** Calculate minimum tokens for Part 2 using linear equation solving. For large
    coordinates (offset by 10^13), we solve the system:

    button_a.move_x * press_a + button_b.move_x * press_b = prize.target_x +
    10^13 button_a.move_y * press_a + button_b.move_y * press_b = prize.target_y
    \+ 10^13

    Using Cramer's rule where determinant is: det = (button_a.move_x *
    button_b.move_y) - (button_b.move_x * button_a.move_y)
...
let calculate_large_coordinate_tokens machines =
  ...
  |> List.fold_left
       (fun total_tokens { button_a; button_b; prize } ->
         ...
         let offset_x = Int64.(add (of_int prize.target_x) 10000000000000L) in
         let offset_y = Int64.(add (of_int prize.target_y) 10000000000000L) in
       ...
       det = (button_a.move_x * button_b.move_y) - (button_b.move_x * button_a.move_y) *)
         ...
         if Int64.equal determinant 0L then total_tokens
           ...
           if
             Int64.(
               equal (rem press_a_numerator determinant) 0L
               && equal (rem press_b_numerator determinant) 0L)
           then
             let press_a = Int64.div press_a_numerator determinant in
             let press_b = Int64.div press_b_numerator determinant in
             (* Step 6: Calculate total cost: press_a * 3 + press_b * 1 *)
             Int64.(add total_tokens (add (mul press_a 3L) press_b))
           else total_tokens)
       0L

(** Parse input string into list of machines. Format example: Button A: X+94,
    Y+34 Button B: X+22, Y+67 Prize: X=8400, Y=5400
...
let parse input =
  let parse_button line expected_button =
    ...
    let pattern =
      Printf.sprintf "^%s: X\\+\\([0-9]+\\), Y\\+\\([0-9]+\\)$" expected_button
    in
    ...
    (* Printf.printf "Debug: Successfully parsed button: x=%d, y=%d\n" button.add_x button.add_y; *)
      ...
      failwith
        (Printf.sprintf "Invalid button format for %s: %s" expected_button line)
  in

  let parse_prize line =
    let pattern = "^Prize: X=\\([0-9]+\\), Y=\\([0-9]+\\)$" in
    let regexp = Str.regexp pattern in
    if Str.string_match regexp line 0 then
      {
    ...
    else failwith (Printf.sprintf "Invalid prize format for Prize: %s" line)
  in
