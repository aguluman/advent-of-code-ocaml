...
let make_register_value_test name register_a register_b register_c program
    expected_reg_a expected_reg_b expected_reg_c =
  name >:: fun _ ->
  let register =
    {
  ...
  | Some regs ->
      assert_equal expected_reg_a
        (Int64.to_int regs.accumulator)
        ~printer:string_of_int ~msg:"Register A";
      assert_equal expected_reg_b
        (Int64.to_int regs.register_b)
        ~printer:string_of_int ~msg:"Register B";
      assert_equal expected_reg_c
        (Int64.to_int regs.register_c)
        ~printer:string_of_int ~msg:"Register C"
  | None -> assert_failure "Program didn't halt properly"

(** Helper to test output-producing programs *)
let make_output_test name register_a register_b register_c program
    expected_output =
  name >:: fun _ ->
  let register =
    {
...
let register_tests =
  [
    ...
    make_register_value_test "B=29, program 1,7 sets B to 26" 0 29 0 [ 1; 7 ] 0
      26 0;
    make_register_value_test "B=2024, C=43690, program 4,0 sets B to 44354" 0
      2024 43690 [ 4; 0 ] 0 44354 43690;
    make_output_test "A=10, program 5,0,5,1,5,4 outputs 0,1,2" 10 0 0
      [ 5; 0; 5; 1; 5; 4 ] [ 0; 1; 2 ];
    make_output_test "A=2024, program 0,1,5,4,3,0 outputs sequence" 2024 0 0
      [ 0; 1; 5; 4; 3; 0 ]
...
let instruction_tests =
  [
    ...
    make_register_value_test
      "bxl instruction performs XOR on register B with literal" 0 10 0 [ 1; 7 ]
      0 (10 lxor 7) 0;
    ...
    make_register_value_test "jnz instruction jumps when A is not zero" 1 0 0
      [ 3; 2; 5; 0; 2; 3 ] 1 3 0;
    (* Should jump to the "bst" and set B=3 *)

    (* jnz instruction doesn't jump when A is zero *)
    make_register_value_test "jnz instruction doesn't jump when A is zero" 0 0 0
      [ 3; 2; 2; 3 ] 0 3 0;
    (* Should execute the "bst" after jnz and set B=3 *)

    (* bxc instruction performs XOR on register B with register C *)
    make_register_value_test
      "bxc instruction performs XOR on register B with register C" 0 10 7
      [ 4; 0 ] 0 (10 lxor 7) 7;
    (* bdv instruction divides A by 2^operand and stores result in B *)
    make_register_value_test
      "bdv instruction divides A by 2^operand and stores in B" 16 0 0 [ 6; 2 ]
      16 4 0;
    (* B = 16 / 2^2 = 16/4 = 4, A remains 16 *)

    (* cdv instruction divides A by 2^operand and stores result in C *)
    make_register_value_test
      "cdv instruction divides A by 2^operand and stores in C" 16 0 0 [ 7; 2 ]
      16 0 4;
    (* C = 16 / 2^2 = 16/4 = 4, A remains 16 *)
  ]
...
let make_part1_test name input expected =
  ...
  assert_equal expected (part1 register program) ~printer:(fun s -> s)
...
let make_part2_test name input expected =
  ...
  assert_equal expected
    (part2 register program |> Int64.to_int)
    ~printer:string_of_int

(** Part 2 test cases *)
let part2_tests = [ make_part2_test "example_2_part2" example_input_2 117440 ]
...
(** Main Entry Point for the chronospatial 3-bit computer simulation
    ...
    This function orchestrates the entire solution workflow: 1. Reads the input
    data from standard input 2. Parses it into register values and program
    instructions 3. Solves part 1 (determining program output with given
    register values) 4. Solves part 2 (finding the register A value that causes
    the program to output itself) 5. Measures and displays the execution time

    Input is expected to be provided via stdin in the format described in the
    parse function. Results are printed to stdout, including:
