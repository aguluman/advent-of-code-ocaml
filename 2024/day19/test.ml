(** Test suite for Day 19: Linen Layout
    Tests the solution for both parts of the challenge using examples from the problem description.
*)
open OUnit2
open Day19

(** Example input from the challenge description *)
let example_input = "r, wr, b, g, bwu, rb, gb, br
...
let make_part1_test name expected_output input =
  name >:: (fun _ ->
    ...
    assert_equal expected_output (part1 (patterns, designs)) ~printer:string_of_int)


(** Part1 test cases *)
let part1_tests = [
  make_part1_test "example_part1" 6 example_input;
]    
...
let make_part2_test name expected_output input =
  name >:: (fun _ ->
    let patterns, designs = parse input in
    assert_equal expected_output (part2 (patterns, designs)) ~printer:Int64.to_string)


(** Part2 test cases *)
let part2_tests = [
  make_part2_test "example_part2" 16L example_input;
]



(** Main test suite *)
let suite = "Day19 Test Suite" >::: [
