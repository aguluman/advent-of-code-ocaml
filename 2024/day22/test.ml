(** Test suite for Day 22: Monkey Market Tests the secret number trading
    challenge using example cases. *)
...
let make_part1_test name input expected =
  ...
  assert_equal expected (part1 initial_secrets) ~printer:Int64.to_string

(** Helper function to create test cases for part2 *)
let make_part2_test name input expected =
  name >:: fun _ ->
  let initial_secrets = parse input in
  assert_equal expected (part2 initial_secrets) ~printer:Int64.to_string
...
(** Tests the solution for part 1 of the Monkey Market challenge.
    ...
    - 100
    - 2024 *)
let part1_tests = [ make_part1_test "example_part1" example_input1 37327623L ]

(** Tests the solution for part 2 of the Monkey Market challenge.

    Validates that our algorithm correctly identifies the optimal sequence of
    four price changes that maximizes the total number of bananas received from
    all buyers.

    The expected result is 23, which represents the sum of bananas obtained by
    following the pattern {b -2,1,-1,3} when selling hiding spots to buyers with
    initial secrets of:
    ...
    - 3
    - 2024 *)
let part2_tests = [ make_part2_test "example_part2" example_input2 23L ]
  ...
  try
    ...
    if has_input then
      ...
      if String.length input > 0 then (
        ...
        initial_secrets |> part1 |> Printf.printf "\nPart 1: %Ld\n%!";

        (* Solve Part 2 with timing *)
        initial_secrets |> part2 |> Printf.printf "Part 2: %Ld\n%!";
