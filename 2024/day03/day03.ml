...
let parse_number text =
  let rec take_digits i =
    if i < String.length text && is_digit text.[i] then take_digits (i + 1)
    else i
  in
...
let parse_prefix prefix text =
  ...
  if String.length text >= prefix_len && String.sub text 0 prefix_len = prefix
  then
...
let parse1 input =
  let rec p text acc =
    if String.length text = 0 then List.rev acc
    else
