  ...
  assert_equal expected_output result ~printer:(fun lst ->
      ...
      ^ String.concat "; "
          (List.map
             (function
               | Multiply (x, y) ->
                   "Multiply(" ^ string_of_int x ^ ", " ^ string_of_int y ^ ")"
               | Enable -> "Enable"
               | Disable -> "Disable")
             lst)
      ^ "]")
...
let part1_tests = [ make_part1_test "testPart1" 161 input1 ]
...
let part2_tests = [ make_part2_test "testPart2" 48 input2 ]
...
let () =
  ...
  if has_input then
    ...
    if String.length input > 0 then (
      ...
      let part1_result = input |> parse1 |> part1 in
      Printf.printf "Part 1: %d\n" part1_result;

      let part2_result = input |> parse2 |> part2 in
