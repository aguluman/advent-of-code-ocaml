(** Test suite for Day 20: Maze Optimization Tests the solution for both parts
    of the challenge using examples. *)
...
let contains_all expected actual =
  ...
  List.iter
    (fun (exp_val, exp_count) ->
      let actual_count = try List.assoc exp_val actual with Not_found -> 0 in
      if actual_count < exp_count then found := false)
    expected;
  !found

(** Helper to create an assertion that checks if results contain all expected
    values *)
let assert_superset_of expected actual =
  if not (contains_all expected actual) then
    assert_failure
      (Printf.sprintf "Expected results to contain all of %s but got %s"
         (String.concat ", "
            (List.map
               (fun (value, count) -> Printf.sprintf "(%d,%d)" value count)
               expected))
         (String.concat ", "
            (List.map
               (fun (value, count) -> Printf.sprintf "(%d,%d)" value count)
  ...
  try
    ...
    if has_input then
      ...
      if String.length input > 0 then (
        ...
        maze |> part1
        |> List.fold_left
             (fun acc (improvement, count) ->
               if improvement >= 100 then acc + count else acc)
             0
        |> Printf.printf "\nPart 1: %d\n%!";
