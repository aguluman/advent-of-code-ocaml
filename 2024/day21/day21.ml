(** Solves Advent of Code Day 21 challenge about finding optimal paths between
    keypad buttons.

    The solution uses dynamic programming with memoization and path optimization
    techniques.

    - Input: A set of button sequences (codes) that need to be navigated
    - Part 1: Calculate the minimum cost to navigate each sequence with a
      recursion level of 3
    - Part 2: Calculate the minimum cost with a much deeper recursion level of
      26
...
let rec make_pairs = function
  | [] | [ _ ] -> []
  | first_button :: second_button :: rest ->
      (first_button, second_button) :: make_pairs (second_button :: rest)
...
(** Retrieves the row and column coordinates for a specified directional keypad
    button.
    ...
    Accepted characters include '^', 'A', '<', 'v', and '>'. Any unrecognized
    ...
    @return
      A tuple (row, column) indicating the button's position in a 2D grid, where
...
(** Calculates possible routes between two numeric buttons on the keypad.
    ...
    Special cases exist for certain button combinations where only one route is
    logical. Each route ends with the 'A' button, representing the
    ...
    @return
      A list of possible routes, where each route is a list of buttons to press
*)
...
let get_direction_button_route source_button target_button =
  let source_row, source_column =
    get_directional_keypad_position source_button
  in
  let target_row, target_column =
    get_directional_keypad_position target_button
  in
  let vertical_moves =
    List.init
      (abs (source_row - target_row))
...
(** Recursively calculates the minimum cost path between two buttons with a
    specified recursion level.

    This function uses a recursive approach to find the optimal path between two
    buttons. It considers both numeric and directional keypads, and uses
...
let rec find_min_cost_path recursion_level source_button target_button =
  if recursion_level = 0 then [ target_button ]
  else
    let is_num_keypad =
      ('0' <= source_button && source_button <= '9')
      || ('0' <= target_button && target_button <= '9')
    in

    let possible_routes =
      if is_num_keypad then get_numeric_button_route source_button target_button
    ...
    let paths =
      List.map
        (fun r ->
          let pairs = 'A' :: r |> fun l -> make_pairs l in
          flatten
            (List.map
               (fun (first_button, second_button) ->
                 find_min_cost_path (recursion_level - 1) first_button
                   second_button)
               pairs))
        possible_routes
    in
    ...
    List.hd
      (List.sort
         (fun first_path second_path ->
           compare (List.length first_path) (List.length second_path))
         paths)

(** Hashtable for memoizing results of the find_min_cost function to avoid
    redundant calculations. *)
let memo = Hashtbl.create 1000

(** Recursively calculates the minimum cost between two buttons with a specified
    recursion level.

    This function uses memoization to optimize performance by caching results of
    previous calculations. The recursion level determines how deep the algorithm
    will analyze possible paths.
    ...
    @param target_button The destination button character
    @return
      The minimum cost (number of button presses) to navigate from source to
      target as an Int64 value *)
  ...
  with Not_found ->
    let value =
      if recursion_level = 0 then 1L
        ...
        let possible_routes =
          if is_num_keypad then
            get_numeric_button_route source_button target_button
          else get_direction_button_route source_button target_button
        in
            ...
            (fun r ->
              ...
              List.fold_left
                (fun acc (first_button, second_button) ->
                  Int64.add acc
                    (find_min_cost (recursion_level - 1) first_button
                       second_button))
                0L pairs)
            possible_routes
        in
...
(** Solves part 1 of the keypad navigation challenge.
    ...
    - This function calculates the total cost for navigating a sequence of
      button codes.
    - Using a recursion level of 3 for path calculation.
    - Each code's cost is multiplied by the numeric value of the code (excluding
      any trailing 'A' characters).
...
let part1 codes =
  Seq.fold_left
    (fun acc code ->
      ...
      let navigation_cost =
        let pairs = 'A' :: code_list |> fun l -> make_pairs l in
        let path =
          flatten
            (List.map
               (fun (first_button, second_button) ->
                 find_min_cost_path (2 + 1) first_button second_button)
               pairs)
        in
      ...
      let num =
        Int64.of_string
          (String.sub code 0
             (String.length code
             - if code.[String.length code - 1] = 'A' then 1 else 0))
      in
...
let parse input =
  ...
  |> List.map String.trim |> Array.of_list |> Array.to_seq
