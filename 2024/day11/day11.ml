...
let blink current_stone =
  if current_stone = 0L then [ 1L ]
    ...
    if digit_count mod 2 = 0 then
      ...
      [
        Int64.of_string (String.sub digit_string 0 half_length);
        Int64.of_string (String.sub digit_string half_length half_length);
      ]
    else [ Int64.mul current_stone 2024L ]
...
let parse input_string =
  input_string |> String.split_on_char ' ' |> List.map Int64.of_string
  |> List.to_seq

(** [part1 input_string] simulates 25 blinks and counts resulting stones
    @param input_string Raw input string from the puzzle
    @return Total number of stones after 25 transformations *)
let part1 input_string =
  let initial_stones = parse input_string in
  let starting_stones = initial_stones |> List.of_seq in
  List.init 25 (fun _ -> ())
  |> List.fold_left
       (fun current_generation _ -> current_generation |> List.concat_map blink)
       starting_stones
  |> List.length

(** [part2 input_string] simulates 75 blinks efficiently using count tracking
    @param input_string Raw input string from the puzzle
    @return Total number of stones after 75 transformations *)
let part2 input_string =
  let initial_stones = parse input_string in
  ...
  List.init 75 (fun _ -> ())
  |> List.fold_left
       (fun generation_map _ ->
         (* Transform each stone and update counts *)
         IntMap.fold
           (fun current_stone frequency new_generation_map ->
             (* Get new stones from current stone *)
             blink current_stone
             |> List.fold_left
                  (fun accumulated_map new_stone ->
                    ...
                    let existing_frequency =
                      ...
                      | None -> 0L
                    in
                    IntMap.add new_stone
                      (Int64.add existing_frequency frequency)
                      accumulated_map)
                  new_generation_map)
  ...
  |> fun final_map ->
  IntMap.fold
    (fun _ stone_count total -> Int64.add total stone_count)
    final_map 0L
