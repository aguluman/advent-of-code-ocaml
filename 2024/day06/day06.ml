...
module StateSet = Set.Make(struct type t = (int * int) * direction let compare = compare end)

(** [parse_obstacles_and_start map] extracts obstacles as a hash table and finds start position. *)
let parse_obstacles_and_start map =
  let n = Array.length map in
  let obstacles = Hashtbl.create (n * n / 4) in  (* Estimate capacity *)
...
(** [get_guard_positions obstacles start width height] returns all positions visited by guard. *)
  ...
  let rec loop () =
    if !x >= 0 && !x < width && !y >= 0 && !y < height then (
      ...
      let rec find_next_move nx ny dir =
        if nx >= 0 && nx < width && ny >= 0 && ny < height && Hashtbl.mem obstacles (nx, ny) then (
  ...
  let rec loop () =
    if !x >= 0 && !x < width && !y >= 0 && !y < height then (
      ...
      if StateSet.mem state !visited then
        ...
        let (dx, dy) = get_direction_offset !direction in        let rec find_next_move nx ny dir =
          if nx >= 0 && nx < width && ny >= 0 && ny < height && Hashtbl.mem obstacles (nx, ny) then (
            let new_dir = next_direction dir in
            let (ndx, ndy) = get_direction_offset new_dir in
            find_next_move (!x + ndx) (!y + ndy) new_dir
...
let part2 map =
  ...
  PosSet.iter (fun pos ->
    ...
    if is_loop new_obstacles start width height then
      incr loop_count
  ) candidates;
  !loop_count
