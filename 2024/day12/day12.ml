...
let parse input =
  ...
  |> List.map String.to_seq |> List.map Array.of_seq |> Array.of_list

(** Find all connected garden plots with the same plant type using Depth-First
    Search
...
let rec find_connected_plots garden_plots (current_row, current_col)
    discovered_plots =
  let adjacent_directions =
    [ (-1, 0); (0, -1); (1, 0); (0, 1) ]
    (* Up, Left, Down, Right *)
  in
  let plots_with_current =
    GardenPlotSet.add (current_row, current_col) discovered_plots
  in

  List.fold_left
    (fun discovered (row_step, col_step) ->
      let next_row = current_row + row_step in
      let next_col = current_col + col_step in

      let is_valid_position =
        next_row >= 0
        && next_row < Array.length garden_plots
        && next_col >= 0
        && next_col < Array.length garden_plots.(current_row)
      in

      let is_same_plant_type =
        is_valid_position
        && garden_plots.(current_row).(current_col)
           = garden_plots.(next_row).(next_col)
      in
      ...
      if is_valid_position && is_same_plant_type && is_undiscovered then
...
(** Calculate total fencing cost for all plant regions in garden
    @param garden_plots The 2D array representing the garden layout
    @param calculate_region_cost
      Function to calculate cost for a specific region
...
let calculate_total_cost garden_plots calculate_region_cost =
  ...
  garden_plots
  |> Array.iter (fun row -> assert (Array.length row = garden_size));

  let garden_coordinates = List.init garden_size (fun index -> index) in
  ...
  let process_plot (total_cost, processed_plots) (row, col) =
    if GardenPlotSet.mem (row, col) processed_plots then
      (total_cost, processed_plots)
    else
      let current_plant_region =
        find_connected_plots garden_plots (row, col) GardenPlotSet.empty
      in
...
let part1 input =
  (* garden_plots: The 2D array of plant types *)
  let garden_plots = parse input in
  let calculate_region_price positions =
    let area = GardenPlotSet.cardinal positions in
    let perimeter =
      GardenPlotSet.fold
        (fun (row, col) perimeter_count ->
          List.fold_left
            (fun count (delta_row, delta_col) ->
              ...
              if
                ...
                && next_row < Array.length garden_plots
                && next_col >= 0
                && next_col < Array.length garden_plots.(next_row)
  ...
  let calculate_region_price positions =
    ...
    let intersections =
      GardenPlotSet.fold
        (fun pos acc ->
          List.fold_left
            (fun set point -> GardenPlotSet.add point set)
    ...
    let count_corner (i, j) =
      ...
      let surrounding =
        List.filter
          (fun pos -> GardenPlotSet.mem pos positions)
          surrounding_points
        |> List.sort compare
      in
    ...
    let corner_count =
      GardenPlotSet.fold (fun pos acc -> acc + count_corner pos) intersections 0
    in
