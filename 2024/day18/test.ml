...
let make_min_steps_test name grid_size bytes expected =
  name >:: fun _ ->
  let positions = parse_byte_positions example_input in
  assert_equal expected
    (calculate_min_steps (grid_size, bytes, positions))
    ~printer:string_of_int

(** Part 1 test cases *)
let part1_tests = [ make_min_steps_test "example_part1" 6 12 22 ]
...
let make_blocking_byte_test name grid_size expected_x expected_y =
  name >:: fun _ ->
  let positions = parse_byte_positions example_input in
  let x, y = find_blocking_byte (grid_size, positions) in
...
(** Part 2 test cases *)
let part2_tests = [ make_blocking_byte_test "example_part2" 6 6 1 ]
...
let grid_visualization =
  ...
  Array.iter
    (fun row ->
      Array.iter
        (fun cell -> Printf.printf "%c" (if cell then '#' else '.'))
        row;
      Printf.printf "\n")
    grid
...
let algorithm_tracing =
  ...
  let positions =
    parse_byte_positions example_input |> Seq.take 5 |> List.of_seq
  in
...
let () =
  try
    (* Check if stdin is available and not empty before trying to read *)
    let has_input =
      try
