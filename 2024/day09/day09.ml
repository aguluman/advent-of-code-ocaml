(** Day 09: Disk Fragmenter
    ...
    Solves the disk defragmentation problem by compacting files and calculating
    checksums.

    {2 Problem Summary:}
    - {b Part 1:} Compact files by moving individual blocks from right to left
      to fill gaps
    - {b Part 2:} Move entire contiguous files to the leftmost available free
      space

    See details at:
    {{:https://adventofcode.com/2024/day/09} Advent of Code 2024, Day 09} *)
...
let create_files_and_spaces disk_map =
  ...
  Array.iteri
    (fun i size ->
      if i mod 2 = 0 then
        (* File *)
        files := (i / 2, { position = !position; size }) :: !files
      else if
        (* Space *)
        size > 0
      then spaces := { position = !position; size } :: !spaces;
...
let checksum_files files =
  files
  |> Array.fold_left
       (fun acc (file_id, file) ->
         let file_sum = ref 0L in
         for pos = file.position to file.position + file.size - 1 do
           file_sum :=
             Int64.add !file_sum
               (Int64.mul (Int64.of_int pos) (Int64.of_int file_id))
         done;
         Int64.add acc !file_sum)
       0L

(** [part1 input] solves part 1 of the challenge - Compact files from right to
    left, filling gaps
...
let part1 input =
  ...
  let rec compact_files remaining_files result_files =
    ...
    | (file_id, file) :: rest_files ->
        ...
        let rec move_file_parts file_remaining current_spaces acc_spaces
            moved_parts =
          if file_remaining.size = 0 then
            ...
            | space :: rest_spaces ->
                if space.position >= file_remaining.position then
                  ...
                  let new_file_part =
                    { position = space.position; size = blocks_to_move }
                  in
                  let remaining_file =
                    {
                      file_remaining with
                      size = file_remaining.size - blocks_to_move;
                    }
                  in
                  let remaining_space =
                    if space.size > blocks_to_move then
                      Some
                        {
                          position = space.position + blocks_to_move;
                  ...
                  move_file_parts remaining_file updated_spaces []
                    ((file_id, new_file_part) :: moved_parts)
                else
                  move_file_parts file_remaining rest_spaces
                    (space :: acc_spaces) moved_parts
        in

        let file_parts, updated_spaces =
          move_file_parts file !spaces_list [] []
        in
...
(** [part2 input] solves part 2 of the challenge - Move entire contiguous files
    to leftmost available space
    ...
    | (file_id, file) :: rest_files -> (
        ...
        let rec find_space current_spaces acc_spaces =
          ...
          | space :: rest_spaces ->
              ...
              else find_space rest_spaces (space :: acc_spaces)
        in
