(** Test suite for Day 25: Schematic Pattern Matching Tests the solution using
    example schematic inputs. *)
...
let make_part1_test name input expected =
  ...
  let converted_schematics =
    List.map (fun schematic -> Array.of_list schematic) schematics
  in
  assert_equal expected (part1 converted_schematics) ~printer:string_of_int

(** Part 1 test cases *)
let part1_tests = [ make_part1_test "example_part1" example_input 3 ]

(** Complete test suite *)
let suite = "Day25 Test Suite" >::: [ "Part 1 Tests" >::: part1_tests ]
