...
let split_updates rules updates =
  ...
  let following =
    List.fold rules ~init:(Map.empty (module Int)) ~f:(fun acc (p, q) ->
      let v = Map.find acc p |> Option.value ~default:(Set.empty (module Int)) in
  ...
  let partition_updates updates =
    let check_updates updates =
      List.for_alli updates ~f:(fun i p ->
        List.for_all (List.drop updates (i + 1)) ~f:(fun q ->
          let p_q = 
          ...
          let q_p =
            ...
            |> Option.value_map ~default:true ~f:(fun v -> not (Set.mem v p))
          in
          p_q && q_p))
    in
...
let parse input =
  ...
  let rules_block, updates_block = 
    match List.group blocks ~break:(fun _ x -> String.is_empty (String.strip x)) with
    | [rules; updates] -> (String.concat ~sep:"\n" rules, String.concat ~sep:"\n" updates)
    | _ -> ("", "")
  in
  ...
  let rules =
    ...
    |> List.filter_map ~f:(fun line ->
      try
        ...
        match parts with
        | [p; q] -> Some (Int.of_string (String.strip p), Int.of_string (String.strip q))
        | _ -> None
      with _ -> None)
  in
  ...
  let updates =
    ...
    |> List.map ~f:(fun line ->
      ...
      |> List.filter_map ~f:(fun x -> 
        try Some (Int.of_string (String.strip x))
        with _ -> None))
  in
